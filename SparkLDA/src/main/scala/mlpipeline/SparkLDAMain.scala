...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    val topic_output = new PrintStream("data/newResults.txt")
    // Load documents, and prepare them for LDA.
    val preprocessStart = System.nanoTime()
   // val (corpus, vocabArray)
    val (corpus, vocabArray) =
     // preprocess(sc, params.input)
      TFIDF(sc, params.input)
   ...
   // topic_output.println(s"\t Training set size: $actualNumTokens tokens")
    ...
    val topics = topicIndices.map { case (terms, weight) =>
      terms.map { case (term) => (vocabArray(term.toInt)) }
    }
    ...
    topics.zipWithIndex.foreach { case (topic, i) =>
      ...
      topics.foreach { case (term) =>
        println(s"${term}")
        topic_output.println(s"TOPIC_$i;$term")
      }
      println()
      topic_output.println()
    }
  ...
  def TFIDF(sc: SparkContext,
                     paths: Seq[String]): (RDD[(Long, Vector)], Array[String]) = {

    val df = sc.textFile(paths.mkString(","))
      .map(_.split(" ").toSeq)

    val hashingTF = new HashingTF()
    val tf: RDD[Vector] = hashingTF.transform(df)
    ...
    //val tfidfIgnore: RDD[Vector] = tfidf.fit(tf)
    ...
    val documentData = df.flatMap(_.toList)
 ...
 // val dd1=dd.distinct().sortBy(_._2,false)
   val StringArray =  dd.map(line => line).collect()
    // $example off$
   ...
   // tfidf.foreach(x => println(x))
    ...
    println("tfidfIgnore: ")
    tfidf.foreach(x => println(x))
    (tfidf.zipWithIndex().map(_.swap), StringArray)
  }
