...
object SparkNaiveBayes {
  ...
  def main(args: Array[String]) {
   // System.setProperty("hadoop.home.dir", "F:\\winutils")
    ...
    val trainFolder = "data/NewsGroups/*"
    ...
    val CATEGORIES = List("atheism", "politcs", "Religion", "science")
    var index = 0
    ...
    var i = 1
    resultData.map(f => {
      owl.createIndividual(":Topic" + f._1, ":Topic")
      println("Printting Topics ---"+topicData.getClass.toString)
      var className = ""
      if (f._2 == 0)
        className = ":atheism"
      else if (f._2 == 1)
